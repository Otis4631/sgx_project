...
int Mat::reshape(vect_int new_shape) {
    int _other = 1;
    int j = 0;
    ...
    if(size % _other == 0)
        new_shape[j] = size / _other;
    if(get_length_from_shape(new_shape) != size) {
        string new_shape_str = shape_to_string(&new_shape);
        string old_shape_str = shape_to_string(&shape);
        string p_str1 = "can not perform reshape to: ";
        string p_str2 = "with old shape: ";
        printf((p_str1 + new_shape_str + p_str2 + old_shape_str).c_str());
        throw exception();
    }
...
Mat Mat::copy() {
     ...
     * 返回当前矩阵的复制
    */
   vect_double _tmp_data(data);
   vect_int _tmp_shape(shape);

   return Mat(_tmp_data, _tmp_shape);
}

void Mat::_indices_new_shape(vector<string>& s, vect_int& res_shape, vector<vect_int>& interval) {
    int iter_shape = -1;
    interval.resize(dimension);
    //res_shape.resize(dimension);
    double _res_shape_tmp[dimension];

    int cnt_interval = 0;
    int cnt_new_dimension = 0;
    ...
    for(auto index_string : s) {
        ...
        vector<string> split_index = split(index_string.c_str(), ":"); //时间占整个函数的20%
        ...
        if(split_index.size() == 1) {  // 只有一个，索引值或起始区间
            ...
            if(pos != index_string.npos) { // 起始区间到结束区间
                ...
                _res_shape_tmp[cnt_new_dimension ++] = (end_pos - start_pos + 1); // 新矩阵的shape 
                array_interval[0] = start_pos; 
                array_interval[1] = end_pos;
                interval[cnt_interval ++] = (array_interval);
            }
            else { // 索引值
            ...
            _res_shape_tmp[cnt_new_dimension ++] = (shape[iter_shape]);
            start_pos = 0;
            ...
            _res_shape_tmp[cnt_new_dimension ++] = (new_length);
            array_interval[0] = start_pos;
            array_interval[1] = end_pos;
            interval[cnt_interval ++] = (array_interval);
        }
    }
    vect_int _tmp(_res_shape_tmp, _res_shape_tmp + cnt_new_dimension);
    res_shape = _tmp;
...
void  Mat::print() {
    char *shape_tmp = new char[BUF_SIZE];
    char *matrix_tmp = new char[BUF_SIZE];
    string matrix_str;
    string shape_str = shape_to_string();
    int _ = 0;
    matrix_to_string(matrix_str, 1, _, 0);
    //strncpy(matrix_tmp, matrix_str.c_str(), BUF_SIZE * 10);
    int expand_buffer_size = BUF_SIZE;
    while(matrix_str.size() > expand_buffer_size && expand_buffer_size < BUF_SIZE * 5) {
        expand_buffer_size *= 2;
    }

    matrix_tmp = new char[expand_buffer_size];
    strncpy(matrix_tmp, matrix_str.c_str(), expand_buffer_size);
...
void Mat::_partition(vect_double& new_data, vector<vect_int>& intervals, int level, int base, int end, int& count) {
    if(level >= dimension)
        return ;
    ...
    for(int block_i = 0; block_i < n_block; block_i ++) {
        ...
        if(level == dimension - 1) {
            for(int i = 0;i <= interval_size;i++) {
                new_data[count ++] = data[base + interval_start + i];
            }
            break;
        }
        else {
            if(block_i >= interval_start && block_i <= interval_end)
                _partition(new_data, intervals, level + 1, base + block_i * block_size, base + (block_i + 1) * block_size, count);
            else if(block_i > interval_end)
                break;
        }
        // if(block_i >= interval_start && block_i <= interval_end) {
        //     if(level == dimension - 1) {
...
Mat Mat::_slice(vect_int& new_shape, vector<vect_int>& intervals) {
    ...
    int count = 0;
    _partition(new_data, intervals, 0, 0, size - 1, count);
...
Mat Mat::operator* (Mat& rmatrix) {
    ...
    data_tmp.resize(res_size);
    for(int i = 0; i < res_size; i++)
        data_tmp[i] = (lmat.data[i] * rmat.data[i]);
...
Mat Mat::operator[](string str) {
    double rnd = rand() % 100;
    printf("PID:%f-----%s\n", rnd, str.c_str());
    const char* hello_mothefuck = str.c_str();
    vector<string> indices ;
    #pragma omp critical
    {
        indices = split(,hello_mothefuck, ",");
    }

    //printf(s);
    if(indices.size() != dimension) {
        //TODO:expection
        printf("%f---can't reslove indices strings since only got %d dimentions, excepted %d \n", rnd,indices.size(), dimension);
        throw exception();
    }
    vect_int new_shape;
    ...
    cout << "slice: " << t2 /(t1 + t2) << "; generate new shape: " << t1 / (t1 + t2)<<": " << t1<< endl;
...
Mat Mat::operator[](const char* s) {
    ...
    if(indices.size() != dimension) {
        //TODO:expection
        printf("can't reslove indices strings since only got %d dimentions, excepted %d \n",indices.size(), dimension);
        throw exception();
    }
    vect_int new_shape;
    vector<vect_int> intervals;
