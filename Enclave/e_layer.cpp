        ...
        for(auto c: range(out_C))
            for(auto h: range(out_H))
                for(auto w: range(out_W)) {
                    ...
                    #else
                        snprintf(char_indices, BUF_SIZE, "%d,%d,%d:%d,%d:%d",
                        i, c, vert_start, vert_end, horiz_start, horiz_end);
                    #endif
...
Conv::Conv(int _in_channels, int _out_channels, int _f, int _stride, int _padding, bool bi, Mat* kernel_data): 
...
Mat Conv::forward(Mat& x) {
    ...
    if(x.shape[1] != weight.shape[1]) {
        printf("excepted channel %d but got %d",weight.shape[1], x.shape[1]);
    throw exception();
    }
    int out_m = x.shape[0];
    ...
    Mat Z = zeros({out_m, out_C, out_H, out_W});
        ...
        std::cout << "pad forward take " << time << " s" << endl;
    #else
        Mat A_prev_pad = pad(x, padding);
    #endif
        ...
        for(int c=0;c<out_C;c++ ) {
               for(int h=0; h < out_H; h ++){
                    ...
                    for(int w = 0; w < out_W; w ++)
                    {
                        ...
                        string weight_index = make_indices("%d,:,:,:", c);
                        //printf("pid:%d, UID:%f---index:%s\n",pid, rnd, weight_index.c_str());
                        Mat _w = weight[weight_index];
                        
                        #ifdef TIME_DEBUG
                        int z_index = c * out_W * out_H + h * out_W + w;
                        time_t s1 = clock();
                        string index = make_indices(":,%d:%d,%d:%d",vert_start,vert_end, horiz_start,horiz_end);
                        time_t m1 = clock();
                        Mat a_slice_prev = a_prev_pad[index];
                        time_t e1 = clock();

                        Mat res_mulit = a_slice_prev * _w;
                        double res_sum = res_mulit.sum();
                        Z.data[z_index] = res_sum + bias.data[c]; // 加法 2s影响
                        time_t e2 = clock();
                        double t1 = (double)(e1 - m1) / CLOCKS_PER_SEC;
                        double t2 = (double)(e2 - e1) / CLOCKS_PER_SEC;
                        double t3 = (double)(m1 - s1) / CLOCKS_PER_SEC; 
                        cout << "mulit:" << t2 /(t1 + t2 +t3) << " indexing:" <<  t1 /(t1 + t2 +t3)<<  " make index: " <<t3 /(t1 + t2 +t3)<< endl;
                        break;
                        #else
                        string index = make_indices(":,%d:%d,%d:%d",vert_start,vert_end, horiz_start,horiz_end);
                        Mat a_slice_prev = a_prev_pad[index];
